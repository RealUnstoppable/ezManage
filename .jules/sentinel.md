...
## 2026-08-01 - Unvalidated Price Override in Stripe Checkout
...
## 2026-08-01 - Duplicate Firestore Rules Bypassing Security Constraints
...
## 2026-08-01 - Cross-Site Scripting (XSS) in index.html Task Priority
...
## 2026-08-01 - Privilege Escalation via Broad Rules in firestore.rules
**Vulnerability:** The `shift_notes` and `employees` collections used combined `allow update, delete:` rules. When attempting to prevent cross-tenant modifications using `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['orgId'])`, applying this diff check to the combined rule would break legitimate `delete` operations, because `delete` operations do not have a `request.resource` payload.
**Learning:** `request.resource` cannot be used inside `delete` operation blocks. A diff check targeting data modification is only valid during an `update` or `create`.
**Prevention:** Always split combined `allow update, delete:` rules into separate `allow update:` and `allow delete:` blocks when applying field-level restrictions to updates.
