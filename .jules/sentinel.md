...
## 2026-07-31 - Unvalidated Price Override in Stripe Checkout
...
## 2026-07-31 - Duplicate Firestore Rules Bypassing Security Constraints
...
## 2026-07-31 - Cross-Site Scripting (XSS) in index.html Task Priority
...
## 2026-07-31 - Cross-Site Scripting (XSS) via Unescaped Error Messages
**Vulnerability:** In `index.html`, error messages caught from promises (`err.message` and `error.message`) were interpolated directly into `innerHTML` strings when displaying error banners for failed fetch operations (e.g., `Failed to load tasks: ${err.message}`).
**Learning:** Even standard error strings (`err.message`) originating from backend systems or external libraries can sometimes contain unescaped payloads or reflect user-controlled input. When injected directly into `innerHTML`, they act as a DOM-based XSS vector.
**Prevention:** Always wrap dynamically rendered variables in `escapeHTML()`, even if they represent standard error messages, to guarantee protection against XSS from unexpected backend responses or payload reflection.
