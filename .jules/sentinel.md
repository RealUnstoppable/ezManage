...
## $(date +%Y-%m-%d) - Unvalidated Price Override in Stripe Checkout
...
## $(date +%Y-%m-%d) - Duplicate Firestore Rules Bypassing Security Constraints
...
## 2025-03-05 - DOM-based XSS in Error Messages
**Vulnerability:** Unescaped error messages (`error.message`) were unsafely interpolated into the DOM using `innerHTML` template literals within error handling blocks (`catch`) across several static HTML and JavaScript files (`index.html`, `js/auth.js`, `js/shop.js`, `js/harmonytunes.js`).
**Learning:** This pattern allowed for Cross-Site Scripting (XSS) payloads to be executed if an attacker could manipulate the backend response or error state to return an error containing malicious script tags, which would then be directly injected into the page structure.
**Prevention:** Always wrap dynamically rendered error strings or external data interpolated into `innerHTML` strings with the `escapeHTML()` utility (found in `js/utils.js`) to ensure all potentially dangerous HTML entities are properly sanitized.
