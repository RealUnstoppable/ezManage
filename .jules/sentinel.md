...
## 2026-05-04 - Privilege Escalation via Client-Side Document Creation
**Vulnerability:** The `allow create` rule for the `/users/{userId}` collection in `firestore.rules` blocked `isBanned` and `membershipLevel` but failed to block `plan` and `subscription`. This allowed an attacker to create their initial user document with premium privileges.
**Learning:** Firestore rules must consistently enforce schema constraints across both `create` and `update` operations. Attackers can inject sensitive fields during the initial document creation if the `create` rule is less restrictive than the `update` rule.
**Prevention:** Ensure that restrictions on privilege and sensitive fields are identical for both `create` and `update` rules. Regularly review all operations (`create`, `update`, `delete`, `read`) to ensure consistent field-level validation and authorization.
