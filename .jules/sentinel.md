...
## $(date +%Y-%m-%d) - Fail-Open Security Controls in XSS Mitigation
**Vulnerability:** A previous attempt to fix a DOM-based XSS vulnerability used a ternary operator to conditionally escape input (`window.escapeHTML ? window.escapeHTML(val) : val`).
**Learning:** This implemented a "fail-open" control. If the security utility was unavailable, the application silently fell back to the original, vulnerable behavior, providing a false sense of security.
**Prevention:** When mitigating security vulnerabilities, never implement 'fail-open' controls. If a security utility is unavailable, the application must fail closed rather than falling back to an insecure, vulnerable state. Better yet, prefer native safe APIs (like `.textContent` or `document.createElement`) which are always available.
