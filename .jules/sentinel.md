...
## $(date +%Y-%m-%d) - Broken Functionality due to Incomplete Event Listener Migration
**Vulnerability:** Refactoring inline event handlers (e.g., `onchange`) into data attributes without registering the corresponding global event listener (`document.addEventListener('change', ...)`) breaks core application functionality.
**Learning:** Security fixes must not break existing functionality. When migrating away from inline event handlers, it is critical to ensure that the new event delegation mechanism accurately captures and routes all intended user interactions.
**Prevention:** Thoroughly map every inline event handler being removed to a specific block in the global event listener logic, and verify the UI interactions manually or via tests post-refactoring.
