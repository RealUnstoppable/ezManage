...
## 2024-05-24 - Unvalidated Price Override in Stripe Checkout
**Vulnerability:** The `createCheckoutSession` Cloud Function accepted an `amount` parameter from the client and used it directly as the `unit_amount` for the Stripe checkout session, allowing attackers to purchase subscriptions for arbitrary low prices.
**Learning:** Never trust the client to dictate the price of a product or service. Price calculation or discount enforcement must be handled securely on the backend.
**Prevention:** Ensure any client-provided `amount` or discount code is validated against a server-side whitelist, minimum threshold, or calculated entirely on the server.
## 2024-05-24 - Duplicate Firestore Rules Bypassing Security Constraints
...
## 2024-05-25 - Unvalidated Price Override in Stripe Checkout Fixed Properly
**Vulnerability:** The `createCheckoutSession` Cloud Function accepted an `amount` parameter from the client and used it directly as the `unit_amount` for the Stripe checkout session, allowing attackers to purchase subscriptions for arbitrary low prices. My initial attempt at fixing this resulted in syntax errors (using `await` in a non-async function context) and the removal of utility functions.
**Learning:** Fixing security bugs must not introduce syntax errors or remove required utilities. Always ensure the fix is structurally sound and tests pass locally before completing pre-commit steps. The server-side code must calculate the final price independently, fetching the necessary user data via Firestore to apply discounts properly without relying on client payloads.
**Prevention:** Ensure any client-provided `amount` or discount code is validated against a server-side whitelist or calculated entirely on the server. Always run `pnpm test` after modifying cloud functions to catch syntax or runtime errors early.
...
## 2026-08-03 - Unvalidated Price Override in Stripe Checkout
**Vulnerability:** The \`createCheckoutSession\` Cloud Function accepted an \`amount\` parameter from the client payload and directly mapped it to the Stripe session's \`unit_amount\`, allowing attackers to purchase premium subscriptions for pennies.
**Learning:** Security-critical financial calculations must never trust values generated by the client. Client requests should only supply state flags (e.g., \`planName\`, \`hasPromoCode\`) which the backend verifies and uses to calculate the final price securely.
**Prevention:** Implement all business logic for discounts and price tiering directly on the server, fetching prerequisite state (like promo code validation) from secure backend data sources (e.g., Firestore) to determine the absolute final cost.

## 2026-08-03 - Duplicate Firestore Rules Bypassing Security Constraints
...
## 2026-08-03 - Cross-Site Scripting (XSS) in index.html Task Priority
...
## 2026-07-07 - Privilege Escalation via Broad Write Rule in firestore.rules\n**Vulnerability:** The `employees` collection used a broad `allow write` rule based solely on the incoming payload's `orgId`. Because `write` implicitly includes `create`, `update`, and `delete`, this rule bypassed the subsequent, more restrictive `allow update, delete` rules. This allowed an attacker to overwrite another organization's employee by supplying their own `orgId` in `request.resource.data.orgId`.\n**Learning:** In `firestore.rules`, a less restrictive operation (like `write`) evaluates before and overrides more granular operations (like `update` or `delete`). Relying on `allow write` for validation allows attackers to bypass strict requirements meant for updates and deletions.\n**Prevention:** Never use the broad `allow write` rule when granular validation (such as ensuring the existing document belongs to the user) is required. Always split `write` operations into specific `allow create`, `allow update`, and `allow delete` rules to enforce precise access controls.
## 2026-07-08 - Cross-Site Scripting (XSS) via Unescaped Error Messages
**Vulnerability:** In `index.html`, error messages caught from promises (`err.message` and `error.message`) in loading routines for incidents, waste logs, and tasks were interpolated directly into `innerHTML` strings (e.g. `Failed to load incidents: ${error.message}`).
**Learning:** Even error messages originating from external libraries or backend systems can sometimes contain unescaped, potentially malicious payloads or reflect user-controlled input, acting as a DOM-based XSS vector when injected into `innerHTML`.
**Prevention:** Always wrap dynamically rendered variables in `escapeHTML()`, even if they represent standard error strings (`err.message`), to guarantee XSS protection against unexpected backend responses or payload reflection.
## 2026-07-09 - Unescaped Promises Error Messages XSS in index.html
**Vulnerability:** Error messages caught from promises (`err.message` and `error.message`) were interpolated directly into `innerHTML` strings (e.g. `Failed to load tasks: ${err.message}`).
**Learning:** Promise errors from external calls or user interactions might contain attacker-controlled inputs if the error source responds with user data. Directly interpolating these unescaped errors into `innerHTML` creates a DOM-based XSS vulnerability.
**Prevention:** Always use a utility function like `escapeHTML()` to sanitize dynamic error messages before rendering them in the DOM via `innerHTML` or similar sinks.
## 2024-05-18 - Fix XSS in HarmonyTunes song rendering
