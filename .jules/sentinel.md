...
## 2025-02-28 - Cross-Site Scripting (XSS) via Unescaped Error Messages
**Vulnerability:** In `index.html`, error messages caught from promises (`err.message` and `error.message`) were interpolated directly into `innerHTML` strings when loading tasks, incidents, and waste logs.
**Learning:** Even error messages originating from external libraries or backend systems can sometimes contain unescaped, potentially malicious payloads or reflect user-controlled input, acting as a DOM-based XSS vector when injected into `innerHTML`.
**Prevention:** Always use safe native DOM manipulation (e.g., `document.createElement` + `textContent`) instead of string interpolation into `innerHTML`, especially for externally-sourced text like error messages.
