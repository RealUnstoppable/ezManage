...
## 2026-08-03 - Splitting Update and Delete Rules for Field Level Restrictions
**Vulnerability:** The \`employees\` collection used a combined \`allow update, delete:\` rule block. If a field-level diff check (like restricting updates to \`orgId\` via \`!request.resource.data.diff(resource.data).affectedKeys().hasAny(['orgId'])\`) was applied to this combined block, it would inadvertently break legitimate delete operations because \`request.resource\` is undefined during deletions.
**Learning:** In Firestore rules, applying payload-dependent constraints (like \`request.resource.data.diff()\`) to combined \`update, delete\` blocks causes unhandled evaluation errors on \`delete\`, blocking users from removing records.
**Prevention:** Always separate \`allow update:\` and \`allow delete:\` into distinct operational blocks before applying granular, field-level restrictions on incoming data updates.
