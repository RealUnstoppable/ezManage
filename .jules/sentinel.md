...
## 2024-05-24 - DOM-based XSS via innerHTML Template Literals
**Vulnerability:** Caught promise error messages (`err.message` or `error.message`) were unsafely interpolated directly into the DOM using `.innerHTML` template literals without sanitization.
**Learning:** This occurs when rendering error states (like 'Failed to load tasks') dynamically. If an attacker can manipulate the backend to return a malicious string in an error response, it will be executed in the victim's browser context.
**Prevention:** Always wrap dynamically rendered error strings in an escaping function (like `escapeHTML()`) before assigning them to `.innerHTML` or use `.textContent` instead if no HTML tags are required.
