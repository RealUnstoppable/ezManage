...
## $(date +%Y-%m-%d) - DOM-based XSS via Inline Event Handlers
**Vulnerability:** Inline `onclick` event handlers with dynamically interpolated variables (`onclick="removeManager('${escapeHTML(doc.id)}')"`).
**Learning:** `escapeHTML` alone cannot prevent XSS inside inline event handlers because the browser parses and unescapes HTML entities inside the attribute string *before* evaluating it as JavaScript.
**Prevention:** Avoid inline execution handlers entirely. Use event delegation by attaching data attributes (`data-action`, `data-id`) and a global event listener, keeping variables securely out of the DOM execution context.
