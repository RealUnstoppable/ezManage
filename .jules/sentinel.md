...
## $(date +%Y-%m-%d) - DOM-based XSS via Unescaped Promise Errors
**Vulnerability:** Error messages caught from promises (`err.message` or `error.message`) were interpolated directly into the DOM using `innerHTML` template literals in `index.html` (e.g. for waste logs, incidents, and tasks).
**Learning:** Even error strings originating from backend responses or external libraries can reflect user-controlled input or contain malicious payloads. Trusting them implicitly and injecting them via `innerHTML` without sanitization creates a DOM-based XSS vector.
**Prevention:** Always wrap dynamically rendered error strings in `escapeHTML()` before assigning them to `innerHTML`, ensuring protection against unexpected injection payloads from backend responses.
