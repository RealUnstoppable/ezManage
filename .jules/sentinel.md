...
## 2026-07-17 - Unvalidated Price Override in Stripe Checkout
...
## 2026-07-17 - Duplicate Firestore Rules Bypassing Security Constraints
...
## 2026-07-17 - Hardcoded API Keys in Client Code
**Vulnerability:** Hardcoded Firebase API keys acting as fallbacks in HTML and configuration files.
**Learning:** Exposing production API keys directly in client-side code, even as fallbacks, presents a critical risk when Security Rules (like Firestore rules) are improperly configured or bypassed, allowing attackers to abuse the project's resources without constraints.
**Prevention:** Always ensure `apiKey` values are sourced exclusively from secure environment variables (`process.env.REACT_APP_FIREBASE_API_KEY`) and never use hardcoded production keys as fallbacks.
## 2026-07-17 - Unvalidated Price Override in Stripe Checkout (`server.cjs`)
**Vulnerability:** The Stripe checkout logic in `server.cjs` contained incomplete price mappings (evaluating to an empty string `""` for the "Business Pro" plan), relying on implicit or missing validations, allowing a bypass of pricing enforcement for premium plans.
**Learning:** Hardcoding logic for payment intent generation must map all tiers to robust, valid constants. Incomplete maps act as logical vulnerabilities that let clients instantiate checkouts with faulty price structures.
**Prevention:** Ensure that all conditionals resolving prices directly on the backend exhaustively map to explicit Stripe price constants, defaulting to the safest or throwing an error if the plan is unrecognized.
