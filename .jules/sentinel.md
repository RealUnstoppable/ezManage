...
## 2026-05-04 - Privilege Escalation via Inconsistent Schema Validation in Firestore
**Vulnerability:** The `/users/{userId}` collection in `firestore.rules` prohibited updates to privilege fields (`plan` and `subscription`) but did not restrict them during initial document creation. This allowed users to inject arbitrary privilege fields when signing up, bypassing the update restriction.
**Learning:** Security restrictions on document schemas must be consistent across both `create` and `update` operations. An attacker can set restricted fields when the document is first created if the `create` rule does not explicitly forbid them.
**Prevention:** Always mirror or combine field-level validation logic (e.g., `hasAny` or `hasOnly`) across both `create` and `update` rules to ensure comprehensive data integrity.
