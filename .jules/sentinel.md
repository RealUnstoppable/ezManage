...
## 2025-02-25 - DOM-based XSS Vulnerability in Error Handling
**Vulnerability:** Unsafe interpolation of error messages (`error.message` or `err.message`) directly into the DOM using `innerHTML` template literals.
**Learning:** Caught promise error messages can sometimes be manipulated by an attacker or backend response, exposing the application to potential cross-site scripting (XSS) if not sanitized.
**Prevention:** Always wrap dynamically rendered error strings in `escapeHTML()` when using `innerHTML` or use safe DOM manipulation methods like `textContent`.
