...
**Vulnerability:** Several variables (like `error.message`, `level`, `song.title`, `product.name`, `k.label`) were dynamically interpolated into `innerHTML` strings without prior escaping across multiple files (`index.html`, `js/auth.js`, `js/harmonytunes.js`, `js/shop.js`, `panel-ai-admin.html`).
**Learning:** Even if data seems benign (like an error message from a library or internal properties), injecting unescaped variables directly into `innerHTML` exposes the application to DOM-based XSS vulnerabilities if an attacker finds a way to control those inputs or if they reflect unexpected payloads.
**Prevention:** Consistently apply `escapeHTML` to all dynamic variables embedded within string literals that are assigned to `innerHTML`, regardless of their source.
