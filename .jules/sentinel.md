...
**Vulnerability:** In `index.html`, error messages caught from promises (`err.message` and `error.message`) were interpolated directly into `innerHTML` strings (e.g. `Failed to load tickets: ${error.message}`).
**Learning:** Even error messages originating from external libraries or backend systems can sometimes contain unescaped, potentially malicious payloads or reflect user-controlled input, acting as a DOM-based XSS vector when injected into `innerHTML`.
**Prevention:** Avoid interpolating variables directly into `innerHTML`. Use native DOM manipulation methods like `document.createElement()` and `textContent`, which automatically escape any malicious payload.
