...
## 2026-09-07 - Fix XSS Vulnerability in Shop Rendering
**Vulnerability:** XSS Vulnerability via innerHTML assignment in `renderProducts()` and `renderCart()`
**Learning:** Directly interpolating dynamically generated strings into the DOM via `innerHTML` opens up potential for cross-site scripting (XSS), even if current data comes from a hardcoded list. If an attacker can inject malicious code, this is dangerous.
**Prevention:** Always use a utility function like `escapeHTML` to correctly escape dynamic variables (e.g. `product.name`, `product.imageUrl`) before passing them to `innerHTML`.
