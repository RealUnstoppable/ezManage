...
## 2024-05-24 - [Timeupdate DOM Assignment Optimization]
**Learning:** High-frequency events (like `timeupdate` on audio players) that unconditionally update DOM properties like `textContent` cause continuous layout thrashing and repaint cycles, even if the formatted value hasn't changed.
**Action:** Always cache the formatted string representation locally and wrap the DOM assignment in a strict equality check (`if (newValue !== lastValue)`) so the browser only repaints when the actual visible content changes.
