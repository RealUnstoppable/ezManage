...
## 2024-05-18 - Sequential Firestore Updates in Webhooks
**Learning:** Sequential updates on multiple documents within loops (such as webhook events processing `snapshot.docs`) block execution until each async network operation concludes, drastically reducing performance and increasing function execution time linearly with the number of documents (N+1 anti-pattern).
**Action:** Replace `for...of` loops performing independent asynchronous work with `.map()` to build an array of Promises, and execute them concurrently using `Promise.all()`. This parallelizes I/O operations and speeds up the entire operation significantly.
