...
## 2024-05-30 - Missing ARIA labels on hardcoded modal close buttons
**Learning:** Hardcoded icon-only close buttons in modals (like the Organization Tutorial Modal) frequently miss `aria-label` attributes because they are often copied and pasted without standard accessibility attributes, making them unreadable by screen readers.
**Action:** Always ensure that any icon-only button used for closing modals or dialogues includes an explicit `aria-label` such as "Close tutorial" or "Close modal" to provide context to screen reader users.
