...
## 2024-07-26 - Avoid redundant ARIA labels
**Learning:** Adding `aria-label`s to elements that already contain descriptive visible text (e.g., `<button>Complete</button>`) creates a redundant or incorrect experience for screen reader users.
**Action:** Only add `aria-label` attributes to elements that truly lack descriptive visible text, such as icon-only interactive elements.
