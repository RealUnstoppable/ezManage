        ...
        function renderOnboardingRoles() {
            ...
            container.innerHTML = onboardingRoles.map((role, i) => `
                <div class="flex justify-between items-center bg-white dark:bg-slate-900 p-3 rounded-lg shadow-sm border border-slate-100 dark:border-slate-800">
                    <span class="font-bold text-sm">${role.email}</span>
                    <div class="flex items-center gap-3">
                        <span class="text-xs uppercase tracking-widest font-black text-slate-400">${role.priority}</span>
                        <button onclick="removeOnboardingRole(${i})" class="text-red-400 hover:text-red-500"><i data-lucide="x" class="w-4 h-4"></i></button>
                    </div>
                </div>
            `).join('');
            lucide.createIcons();
                ...
                snap.forEach(doc => {
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        <td class="px-6 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="font-black text-slate-800 dark:text-slate-200">
                                    ${displayName} ${isOwner ? '<span class="ml-2 text-[10px] bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 px-2 py-0.5 rounded-full uppercase tracking-widest">Admin</span>' : ''}
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-3 text-slate-500 text-xs font-bold">${d.email || 'N/A'}</td>
                        <td class="px-6 py-3">
                        ...
                        <td class="px-6 py-3 text-right">
                            ${!isOwner ? `<button onclick="removeManager('${doc.id}')" class="text-slate-400 hover:text-red-500 transition-colors"><i data-lucide="trash-2" class="w-4 h-4"></i></button>` : `<span class="text-slate-300">-</span>`}
                        </td>
                    </tr>`;
                ...
                snap.forEach(doc => {
                    ...
                    <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-start gap-4 hover:border-${color}-200 dark:hover:border-${color}-800 transition-colors">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-white flex-shrink-0 mt-1 font-black shadow-sm" style="background-color: ${authorColor}">
                            ${(data.authorName || 'U').substring(0, 1).toUpperCase()}
                        </div>
                        <div class="flex-grow">
                            <div class="flex justify-between items-start mb-1">
                                <div class="font-black text-slate-900 dark:text-white text-sm">${data.authorName}</div>
                            ...
                            <div class="text-sm text-slate-600 dark:text-slate-400 mb-2">${data.content}</div>
                            <div class="text-[10px] font-bold uppercase tracking-widest text-slate-400">Status: ${data.status}</div>
                        </div>
                    </div>`;
                });
                ...
                .onSnapshot(snap => {
                    ...
                    snap.forEach(doc => {
                        ...
                        <div class="flex justify-between items-center p-3 bg-white dark:bg-slate-800 border border-amber-200 dark:border-amber-900/50 rounded-xl shadow-sm">
                            <div>
                                <div class="font-black text-slate-900 dark:text-white text-sm">${doc.data().userName}</div>
                                <div class="text-[10px] font-bold uppercase tracking-widest text-amber-500">Requested Access</div>
                            </div>
                            <button onclick="approveRequest('${doc.id}')" class="btn btn-accent btn-sm py-1 px-4 text-xs">Approve</button>
                        </div>
                      `;
                    });
        ...
        async function removeManager(userId) {
            ...
            alert("Removing managers requires backend Cloud Function support which is coming in v2!");
        }
