...
## 2024-05-24 - ARIA labels for dynamic task deletion buttons
**Learning:** Icon-only buttons representing destructive actions, such as dynamically injected "Delete Assigned Task" buttons that only utilize Lucide icons (`<i data-lucide="trash-2">`), require explicit `aria-label` attributes to ensure they are properly announced to screen reader users, rather than being generically labeled as "button".
**Action:** Consistently review dynamically generated task or list items in template literals (e.g. `index.html`) to ensure any icon-only action buttons are equipped with context-specific `aria-label`s.
## 2026-09-17 - Updating IDs correctly
**Learning:** When trying to align suffix numbering in HTML forms (like replacing `taskAssignee1` with `taskAssignee2`), it's very easy to break the Javascript validation and logic functions that expect the original generic IDs (`taskAssignee`). Doing global search and replaces without understanding the surrounding forms can cause severe regressions (where updating one form's suffix breaks the submission handler of *other* forms).
**Action:** When updating suffixes to align numbering, strictly scope Javascript updates to the specific function handling that form submission, ensuring NOT to mutate global logic or break other active components relying on overlapping or separate IDs.
