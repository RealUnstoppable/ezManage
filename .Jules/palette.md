...
## 2024-11-20 - Adding `for` attributes to inputs in forms
**Learning:** For a simple UX improvement, providing missing `for` attributes inside `label` tags to map directly to input `id` attributes greatly improves screen reader compatibility and provides a larger click target for the inputs. However, we should be extremely careful not to pull in out-of-scope changes such as dependency bumps by accidentally running `npm install`.
**Action:** When working as the Palette agent, always ensure the PR limits itself strictly to HTML/CSS accessibility and UX improvements, and restore unintended `package-lock.json` changes.
